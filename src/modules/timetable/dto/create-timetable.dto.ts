import { IsString, IsNotEmpty, IsOptional, IsUUID, IsBoolean, IsInt, IsArray, ValidateNested, Min, Max, MaxLength } from 'class-validator';
...
export class CreateTimetableDto {
  ...
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => PeriodTimeDto)
  period_times?: PeriodTimeDto[];

  @ApiPropertyOptional({ type: [TimetableEntryDto] })
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => TimetableEntryDto)
