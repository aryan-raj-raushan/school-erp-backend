...
import { Timetable, NewTimetable, NewTimetablePeriodTime, TimetablePeriodTime, NewTimetableEntry, TimetableEntry, NewTimetableClassTeacher, TimetableClassTeacher } from './types/timetable.types';
...
export class TimetableRepository {
  ...
  async findAll(schoolId: string, filters: { academic_year_id?: string; class_id?: string; class_detail_id?: string }) {
    ...
    if (filters.class_detail_id) conditions.push(eq(timetables.class_detail_id, filters.class_detail_id));

    return this.db
      .select({
        id: timetables.id, school_id: timetables.school_id, academic_year_id: timetables.academic_year_id,
        class_id: timetables.class_id, class_detail_id: timetables.class_detail_id,
        name: timetables.name, max_periods: timetables.max_periods, is_complete: timetables.is_complete,
        deleted: timetables.deleted, created_at: timetables.created_at, updated_at: timetables.updated_at,
        class_name: classes.name, class_detail_name: classDetails.name,
      })
      .from(timetables)
  ...
  async findById(id: string, schoolId: string) {
    const [row] = await this.db
      ...
      .leftJoin(classes, eq(timetables.class_id, classes.id))
      .leftJoin(classDetails, eq(timetables.class_detail_id, classDetails.id))
      .where(and(eq(timetables.id, id), eq(timetables.school_id, schoolId), eq(timetables.deleted, false)));
    return row;
  }
  ...
  async update(id: string, schoolId: string, data: Partial<NewTimetable>): Promise<Timetable> {
    const [row] = await this.db.update(timetables).set({ ...data, updated_at: new Date() }).where(and(eq(timetables.id, id), eq(timetables.school_id, schoolId))).returning();
    return row;
  }

  async softDelete(id: string, schoolId: string): Promise<void> {
    await this.db.update(timetables).set({ deleted: true, updated_at: new Date() }).where(and(eq(timetables.id, id), eq(timetables.school_id, schoolId)));
  }

  // Period times
  async findPeriodTimes(timetableId: string): Promise<TimetablePeriodTime[]> {
    return this.db.select().from(timetablePeriodTimes).where(eq(timetablePeriodTimes.timetable_id, timetableId)).orderBy(timetablePeriodTimes.period_number);
  }

  async replacePeriodTimes(timetableId: string, data: NewTimetablePeriodTime[]): Promise<TimetablePeriodTime[]> {
    await this.db.delete(timetablePeriodTimes).where(eq(timetablePeriodTimes.timetable_id, timetableId));
  ...
  async findEntries(timetableId: string) {
    return this.db
      .select({
        id: timetableEntries.id, timetable_id: timetableEntries.timetable_id,
        school_id: timetableEntries.school_id, day_of_week: timetableEntries.day_of_week,
        period_number: timetableEntries.period_number, subject_id: timetableEntries.subject_id,
        teacher_id: timetableEntries.teacher_id, created_at: timetableEntries.created_at,
  ...
  async replaceEntries(timetableId: string, schoolId: string, data: Omit<NewTimetableEntry, 'id' | 'school_id' | 'created_at'>[]): Promise<TimetableEntry[]> {
    await this.db.delete(timetableEntries).where(eq(timetableEntries.timetable_id, timetableId));
    if (!data.length) return [];
    const rows = data.map((d) => ({ ...d, id: require('../../utils/uuid.utils').generateId(), school_id: schoolId, timetable_id: timetableId }));
  ...
  async setClassTeacher(timetableId: string, schoolId: string, teacherId: string) {
    await this.db.delete(timetableClassTeachers).where(eq(timetableClassTeachers.timetable_id, timetableId));
    const [row] = await this.db.insert(timetableClassTeachers).values({
      id: require('../../utils/uuid.utils').generateId(), timetable_id: timetableId, school_id: schoolId, teacher_id: teacherId,
    }).returning();
    return row;
  }
  ...
  async findEntriesByTeacher(schoolId: string, teacherId: string) {
    return this.db
      ...
      .from(timetableEntries)
      .innerJoin(timetables, and(eq(timetableEntries.timetable_id, timetables.id), eq(timetables.school_id, schoolId), eq(timetables.deleted, false)))
      .leftJoin(subjects, eq(timetableEntries.subject_id, subjects.id))
      .leftJoin(classes, eq(timetables.class_id, classes.id))
      .leftJoin(classDetails, eq(timetables.class_detail_id, classDetails.id))
      .where(and(eq(timetableEntries.school_id, schoolId), eq(timetableEntries.teacher_id, teacherId)))
      .orderBy(timetableEntries.day_of_week, timetableEntries.period_number);
  }
