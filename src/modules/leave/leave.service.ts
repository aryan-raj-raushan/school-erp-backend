...
export class LeaveService {
  ...
  async applyTeacherLeave(
    ...
    return this.leaveRepo.createTeacherRequest({
      id: generateId(),
      school_id: schoolId,
      staff_id: staffId,
  ...
  async reviewTeacherLeave(
    ...
    return this.leaveRepo.reviewTeacherRequest(requestId, schoolId, {
      status: dto.status,
  ...
  async reviewStudentLeave(
    ...
    if (!req) throw new NotFoundException(`Leave request with id '${requestId}' not found`);
    return this.leaveRepo.reviewStudentRequest(requestId, schoolId, {
      status: dto.status,
      reviewed_by: reviewerId,
      reviewer_remarks: dto.reviewer_remarks,
    });
  ...
  async applyStudentLeave(
    ...
    const to = new Date(dto.to_date);
    const totalDays = Math.ceil((to.getTime() - from.getTime()) / (1000 * 60 * 60 * 24)) + 1;
    return this.leaveRepo.createStudentRequest({
      id: generateId(),
      school_id: schoolId,
