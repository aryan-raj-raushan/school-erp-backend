...
export class SchoolsRepository {
  ...
  async findAll(filters: SchoolFilterDto): Promise<School[]> {
    ...
    if (filters.board_type) {
      conditions.push(eq(schools.board_type, filters.board_type));
    }
    if (filters.is_active !== undefined) {
      conditions.push(eq(schools.is_active, filters.is_active));
    }
  ...
  async count(filters: SchoolFilterDto): Promise<number> {
    const conditions = [eq(schools.deleted, false)];
