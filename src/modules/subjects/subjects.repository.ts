...
export class SubjectsRepository {
  ...
  private buildConditions(schoolId: string, filters: Partial<SubjectFilterDto> = {}) {
    const conditions = [eq(subjects.school_id, schoolId), eq(subjects.deleted, false)];
  ...
  async findById(id: string, schoolId: string): Promise<Subject | undefined> {
    const [row] = await this.db
      ...
      .from(subjects)
      .where(
        and(eq(subjects.id, id), eq(subjects.school_id, schoolId), eq(subjects.deleted, false)),
      );
    return row;
  }
