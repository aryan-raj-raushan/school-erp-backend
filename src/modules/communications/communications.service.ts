...
export class CommunicationsService {
  ...
  private notifKey(schoolId: string) {
    return `notifications:${schoolId}`;
  }
  private commKey(schoolId: string) {
    return `communications:${schoolId}`;
  }
  ...
  async sendNotification(
    ...
    const notif = await this.repo.createNotification({
      ...
      recipient_role: dto.recipient_role,
    });
  ...
  async getSentByUser(senderId: string, schoolId: string): Promise<Notification[]> {
    ...
    return this.redisService.getOrSet(key, CacheTTL.MEDIUM, () =>
      this.repo.findSentByUser(senderId, schoolId),
    );
  }
  ...
  async sendCommunication(
    ...
    const comm = await this.repo.createCommunication({
      ...
      sender_id: senderId,
      recipient_id: dto.recipient_id,
      type: dto.type as Communication['type'],
      subject: dto.subject,
      message: dto.message,
    });
  ...
  async getCommunications(
    schoolId: string,
    userId: string,
    view: 'sent' | 'received' | 'all' = 'all',
  ): Promise<Communication[]> {
    const key = `${this.commKey(schoolId)}:user:${userId}:${view}`;
    return this.redisService.getOrSet(key, CacheTTL.MEDIUM, () =>
      this.repo.findCommunications(schoolId, userId, view),
    );
  }
  ...
  async reviewCommunication(
    ...
    schoolId: string,
    reviewerId: string,
    dto: ReviewCommunicationDto,
  ): Promise<Communication> {
    const comm = await this.repo.findCommunicationById(id, schoolId);
    if (!comm) throw new NotFoundException(`Communication with id '${id}' not found`);
    const updated = await this.repo.reviewCommunication(id, schoolId, {
      status: dto.status as Communication['status'],
      reply: dto.reply,
