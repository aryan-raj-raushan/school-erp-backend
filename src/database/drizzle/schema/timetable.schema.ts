...
export const timetables = pgTable('timetables', {
  ...
  academic_year_id: varchar('academic_year_id', { length: 36 }).references(() => academicYears.id, {
    onDelete: 'set null',
  }),
...
export const timetablePeriodTimes = pgTable('timetable_period_times', {
  id: varchar('id', { length: 36 }).primaryKey(),
  timetable_id: varchar('timetable_id', { length: 36 })
    .notNull()
...
export const timetableEntries = pgTable('timetable_entries', {
  ...
  period_number: integer('period_number').notNull(),
  subject_id: varchar('subject_id', { length: 36 }).references(() => subjects.id, {
    onDelete: 'set null',
  }),
  teacher_id: varchar('teacher_id', { length: 36 }).references(() => schoolUsers.id, {
    onDelete: 'set null',
  }),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
});
