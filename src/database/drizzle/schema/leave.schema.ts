...
export const leaveBalances = pgTable('leave_balances', {
  ...
  carried_forward: integer('carried_forward').default(0).notNull(),
  expires_at: date('expires_at'),
  auto_credited_at: timestamp('auto_credited_at', { withTimezone: true }),
  can_go_negative: boolean('can_go_negative').default(false).notNull(),
  academic_year_id: varchar('academic_year_id', { length: 36 }).notNull(),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updated_at: timestamp('updated_at', { withTimezone: true }),
});
